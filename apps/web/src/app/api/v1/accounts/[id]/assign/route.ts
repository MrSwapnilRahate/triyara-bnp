import { assignOwnerSchema } from '@triyara/validation'
...
export function POST(req: Request, { params }: Params) {
  return route(req, async (requestId) => {
    ...
    const dto = await parseBody(req, assignOwnerSchema)
    const account = await accountService.assign({ ...auth, requestId }, id, dto, version)
