import { requestDocumentsSchema } from '@triyara/validation'
...
export function POST(req: Request, { params }: Params) {
  return route(req, async (requestId) => {
    ...
    const dto = await parseBody(req, requestDocumentsSchema)
    const v = await verificationService.requestDocuments({ ...auth, requestId }, id, dto, version)
