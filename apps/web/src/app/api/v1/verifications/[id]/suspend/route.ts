import { reasonSchema } from '@triyara/validation'
...
export function POST(req: Request, { params }: Params) {
  return route(req, async (requestId) => {
    ...
    const dto = await parseBody(req, reasonSchema)
    const v = await verificationService.suspend({ ...auth, requestId }, id, dto, version)
