import { approveVerificationSchema } from '@triyara/validation'
...
export function POST(req: Request, { params }: Params) {
  return route(req, async (requestId) => {
    ...
    const dto = await parseBody(req, approveVerificationSchema)
    const v = await verificationService.approve({ ...auth, requestId }, id, dto, version)
